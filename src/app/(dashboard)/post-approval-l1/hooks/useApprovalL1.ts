...
export interface UseApprovalL1Return {
  // State
  searchQ: string
  ...
  submitStatus: 'PENDING_L2' | 'REJECTED' | null
  setSubmitStatus: (status: 'PENDING_L2' | 'REJECTED' | null) => void
  note: string
  ...
  // Actions
  openResultDialog: (post: ApprovalPost, status: 'PENDING_L2' | 'REJECTED') => void
  handleOpenHistoryDialog: () => void
  handleSubmitResult: () => Promise<void>
  clearActivePost: () => void
}

export function useApprovalL1(): UseApprovalL1Return {
  ...
  const [submitStatus, setSubmitStatus] = React.useState<'PENDING_L2' | 'REJECTED' | null>(null)
  const [note, setNote] = React.useState('')

  const canApprovePost =
    ability.can('approve_post', 'post-approval-1') || ability.can('approve_post', 'news')
  const canViewPost = ability.can('view_post', 'post-approval-1')
  const canViewHistory =
    ability.can('view_history', 'post-approval-1') || ability.can('view_history', 'news')

  const currentStatus = L1_STATUS_MAP[activeTab]
  ...
  const {
    ...
    queryKey: ['post-approval-l1', activeTab, searchQ],
  ...
  const openResultDialog = useCallback(
    (post: ApprovalPost, status: 'PENDING_L2' | 'REJECTED') => {
      if (!canApprovePost) {
        toast.error({
