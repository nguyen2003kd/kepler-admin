...
export function GalleryConfig() {
  ...
  const handleSaveAllChanges = async () => {
    ...
    try {
      ...
      // Check if there are any cropped images to upload
      if (croppedBlobs.size > 0) {
        toast.warning({ title: "Đang xử lý", content: "Đang upload ảnh vào kho..." });

        // Upload all cropped images to the gallery
          ...
          async ([index, blob]) => {
            try {
              ...
              if (result.responseData?.path) {
                const uploadedUrl = result.responseData.path;
        ...
        // Update page config with new image URLs
      ...
      if (contentData.title !== tempContentData.title && contentData.titleId) {
        await updateConfigMutation.mutateAsync({
      ...
      if (contentData.description !== tempContentData.description && contentData.descriptionId) {
        await updateConfigMutation.mutateAsync({
      ...
      toast.success({ title: "Thành công", content: "Đã lưu gallery thành công!" });
      setHasChanges(false);
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              >
                ...
                {isEditingContent ? "Hủy" : "Sửa nội dung"}
              </Button>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              {/* Column 1 - Image 1 (30%) + Image 2 (70%) */}
              ...
              <div className="lg:col-span-2 grid grid-cols-1 lg:grid-cols-[35%_65%] gap-4">
                ...
                {/* Column 3 - Text Content (70% height) */}
                <div className="flex flex-col justify-start space-y-4 text-gray-900 p-6 bg-gray-50 rounded-lg">
                  {isEditingContent ? (
                    ...
                    <>
                      <h2
                        className="text-4xl lg:text-5xl font-semibold leading-relaxed tracking-wider whitespace-pre-line"
                ...
                {/* Image 4 - Spans both columns (30% height) */}
                {bannerSlides[3] && (
